...
const buildVisionTokenSet = (vision = {}) => {
    ...
    if (vision.attributes?.product_type) add(vision.attributes.product_type);
    if (vision.attributes?.category) add(vision.attributes.category);
    if (vision.attributes?.color) add(vision.attributes.color);
    if (vision.attributes?.material) add(vision.attributes.material);
    return tokens;
};
